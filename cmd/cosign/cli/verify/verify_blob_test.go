...
func TestVerifyBlob(t *testing.T) {
	...
	tts := []struct {
		...
		{
			name:         "valid signature with public key - bundle without rekor bundle ok",
			blob:         blobBytes,
			signature:    blobSignature,
			sigVerifier:  signer,
			experimental: false,
		...
		{
			name:         "valid signature with short-lived cert - bundle without rekor bundle fails",
			blob:         blobBytes,
			signature:    blobSignature,
			sigVerifier:  signer,
			cert:         expiredLeafCert,
			experimental: false,
			bundlePath:   makeLocalBundleWithoutRekorBundle(t, []byte(blobSignature), expiredLeafPem),
			shouldErr:    true,
		},
		{
			name:         "valid signature with short-lived cert - bundle without rekor bundle succeeds with rekor lookup",
			blob:         blobBytes,
			signature:    blobSignature,
			sigVerifier:  signer,
			experimental: true,
